...
- 사전지식
  ...
  - Diameter
    - Social Network에서 가장 긴 Social Distance의 길이(두 노드 사이의 엣지의 길이가 아닌 한 노드에서 다른 노드로 가기 위해 거쳐야 하는 노드의 개수)
  - Length of Network(L)
    - 모든 social distance(한 노드에서 다른 노드로 가기 위해 거쳐야 하는 노드의 개수)의 중간값. 일반적으로 그래프의 크기가 커지면 같이 커진다.
...
- NSW(Navigable Small World graph)
  ...
  - 원리
    - 두 개의 연결 된 노드  N1, N2가 있다.
    - 두 노드를 연결하는 엣지의 중간에 노드 N3가 추가된다.
    - 기존에는 N1에서 N2로 갈 때, 1번만에 갈 수 있었지만, 이제는 N3를 거쳐서 짧은 거리를 2번 이동해야 한다.
    - 만일 N1과 N2 사이에 N3가 추가되어도 N1과 N2를 연결하지 않는다면 여전히 N1에서 N2까지 한 번만에 갈 수 있다.
  - 검색 과정
