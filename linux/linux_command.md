...
- 사용자 삭제
  ...
  - 옵션
    - `-r`: 사용자를 삭제하면서 홈 디렉토리도 삭제한다.

  ```bash
  $ bash userdel [옵션] <사용자명>
  ```
...
- 소유권 확인
  ...
  - 출력 예시
    - 순서대로 파일 종류 및 권한, 링크 수, 사용자(소유자), 그룹, 파일 크기, 수정 시간, 파일 이름을 나타낸다.

  ```bash
  $ ls -l
  total 0
  drwxr-xr-x 2 root    root     6 Apr  1  2021 etc
  drwxr-xr-x 5 root    root    60 Sep  7 15:24 elasticsearch
  drwxr-xr-x 2 root    root     6 Oct 27 09:40 hedge
  ```

  - 파일 종류 및 권한
    - 크게 `d`, `rwx`, `r-x`, `r-x` 네 부분으로 나눌 수 있다.
    - 첫 번째 `d`는 디렉터리라는 의미이며 파일일 경우 `d`가 아닌 `-`가 온다.
    - 두 번째는 사용자(소유자)의 권한을 의미하며 `r`은 읽기, `w`는 쓰기, `x`는 실행을 의미한다.
    - 세 번째, 네 번째는 각기 그룹 권한과 다른 사용자의 권한을 의미하며 여기에 쓰인 `-`는 해당 권한이 없음을 의미한다.
    - 즉 `drwxr-xr-x`는 `etc`는 디렉토리이며, 사용자는 읽고, 쓰고, 실행이 가능하지만 그룹과 다른 사용자는 읽고 실행만 가능한 파일이라는 의미이다.
...
- 파일 권한 변경
  ...
  - `chomd` 명령을 사용한다.
  - `u`, `g`, `o`, `a`와 `+`, `-`, 그리고 `x`, `r`, `w`를 조합하여 사용한다.
