...
- 사용 예시
  ...
  */
  ...
  //관리자 도구에서 Vue탭을 보면 넘겨받은 데이터,이벤트 등이 payload에 담겨서 넘어오게 된다. 아래에 payload라고 적은 것은인자 이름이므로, payload라고 적는 것 보다는 실제 payload에 담겨 넘어온 것을 명시적으로 알 수 있도록 하는 것이 더 낫다.
  export default new Vuex.Store({
    ...
    mutations: {
      ...
      //mutations의 존재 이유가 state(data)의 변경이기 때문이다.
