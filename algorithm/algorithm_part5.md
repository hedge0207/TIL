...
- 분할 정복
  ...
  - 복잡한 문제를 바로 해결할 수 있을 정도로 간단한 문제가 될 때까지 문제를 재귀적으로 분할한 다음, 그 하위 문제들의 결과들을 조합하여 원래 문제의 결과로 만드는 알고리즘이다
    - 분할 정복을 활용한 대표적인 알고리즘이 병합 정렬 알고리즘이다.
  - 분할, 정복, 조합의 과정을 거친다.
    - 복잡한 문제를 점차 단순한 문제로 분할한다.
    - 단순화된 부분 문제들을 정복한다.
    - 부분 문제의 답들을 조합한다.
  - 수도 코드
...
- 다이나믹 프로그래밍(Dynamic Programming, DP)
  - 응용 수학자 리차드 벨만이 1953년 고안한 알고리즘이다.
  ...
  - DP의 핵심은 중복된 하위 문제들의 결과를 저장해두는 것이다.
...
- 다이나믹 프로그래밍 방법론
  ...
  - 하향식(Memoization)
    - 하위 문제에 대한 정답을 계산했는지 확인해가며 문제를 풀어나가는 방식이다.
    - 
