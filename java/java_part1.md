...
- 좋은 객체지향 설계의 5가지 원칙(SOLID)
  ...
  - OCP: 개방-폐쇄 원칙(Open/closed principle)
    ...
    - 그러나 구현 객체를 변경하려면 클라이언트도 변경해야 하는 경우도 존재하는데, spring에서 이를 해결해 주는 역할을 하는 것이 컨테이너이다. 의존성 주입(DI)도 이 원칙을 유지하면서 변경을 하기 위해 사용하는 것이다.
  ...
  - ISP: 인터페이스 분리 원칙(Interface segregation principle)
    ...
    - 예를 들어 자동차라는 범용 인터페이스에서 정비와 운전이라는 역랗을 모두 수행하는 것 보다는 운전 인터페이스와 정비 인터페이스를 나누는 것이 더 낫다.
  ...
  - DIP: 의존관계 역전 원칙(Dependency inversion principle) 
    ...
    - 그러나 인터페이스에 의존하지만 구현 클래스에도 의존해야하는 상황도 존재하는데, spring의 경우 이 역시 컨테이너가 이를 해결하는 역할을 해준다.
...
- 다형성(Polymorphism)
  ...
  - 다형성의 본질
    - 인터페이스를 구현한 객체인스턴스를 싱행하는 시점에서 유연하게 변경할 수 있다.
